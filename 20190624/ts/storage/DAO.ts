...
abstract class DAO {
     ...
     * @returns void
     */
    abstract soundSphereDBToJson(bufferList: ItemBuffer[], listItemMixPanel: ItemMixPanel[][], listSemanticDescriptors: SemanticDescriptor[], soundSphereInfo: SoundSphereInfo, sessionControl: SessionControl): void;
    constructor(soundSphereInfo: SoundSphereInfo, listSemanticDescriptors: SemanticDescriptor[], audioCtx: AudioContext | any, controlFiles: ControlFiles, sessionControl: SessionControl) {
    ...
    loadBufferList(listAudioData: any, callback: any, newList: boolean = true) {
        ...
        for (let index = 0; index < listAudioData.length; index++) {
            this.audioCtx.decodeAudioData(listAudioData[index].buffer).then(
                (buffer: any) => {
     ...
     * @param  {ItemMixPanel[][]} listItemMixPanel
     */
    downloadJSON(nameFileUser: string) {
