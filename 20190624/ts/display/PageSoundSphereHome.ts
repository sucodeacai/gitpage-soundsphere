class PageSoundSphereHome extends SimplePage {
  ...
  mouseOutIconAlbum: boolean = true
  tooltip: Tooltip
  painel!: Painel
  ...
  setSettingsActions(): void {
    ...
    $('#buttonVoiceComand').on('click', (e: JQueryEventObject) => {
      $('#buttonVoiceComand').toggleClass("active");
      this.voiceCommandMenuBar!.startRecognition(this.onEndRecordMenuBar)
    });
    $('#restartPanel').on('click', (e: JQueryEventObject) => {
      this.showModalRestartPanel()
    });
    $('#buttonDownload').on('click', (e: JQueryEventObject) => {
      this.showModalDownloads()
    });
    $("#nameFile").attr("placeholder", this.dao.getDefaultName());
    $('#buttonLoop').on('click', (e: JQueryEventObject) => {
      this.sequenciador.changeLoop();
      $('#buttonLoop').toggleClass("active");
    });
    $('#buttonPlay').on('click', (e: JQueryEventObject) => {
      this.playMixagem();
    });
    $('#buttonPause').on('click', (e: JQueryEventObject) => {
      this.pauseMixagem();

    });
    $('#buttonStop').on('click', (e: JQueryEventObject) => {
      this.stopMixagem();
    });
    $('#cancelModal').on('click', (e: JQueryEventObject) => {
      $('.ui.modal').modal('hide');
    });
    $('#confirmRestartPanel').on('click', (e: JQueryEventObject) => {
      this.sequenciador.stop(function () { });
      this.dao.restartMixing();
      this.painel.reMake();
      $('.ui.modal').modal('hide');
    });
    $('#buttonDownloadWav').on('click', (e: JQueryEventObject) => {
      this.sequenciador.startDownload(this.closeModalDownload, $("#nameFile").val());
      this.generateHTML();
    });
    $('#buttonDownloadJson').on('click', (e: JQueryEventObject) => {
      this.dao.downloadJSON($("#nameFile").val());
      this.closeModalDownload();
      this.generateHTML();
    });
    $('#buttonDownloadJsonWav').on('click', (e: JQueryEventObject) => {
      this.dao.downloadJSON( $("#nameFile").val());
      this.sequenciador.startDownload(this.closeModalDownload, $("#nameFile").val())
      this.generateHTML();
    });
    ...
    $('#buttonInitialUploadWav').on('click', (e: JQueryEventObject) => {
      this.sequenciador.stop(function () { });
      $("#filesWav").click()
      this.closeModalInitial()
    });
    $('#buttonJson1Cancelar').on('click', (e: JQueryEventObject) => {
      this.closeModalJson1()
      this.backToMainModal();
    });
    $('#buttonJson2Cancelar').on('click', (e: JQueryEventObject) => {
      this.closeModalJson2()
      this.backToMainModal()
    });

    $('#buttonJson1SelctJson').on('click', (e: JQueryEventObject) => {
      $('#fileJSON').click()
    });

    $('#buttonJson2SelctWav').on('click', (e: JQueryEventObject) => {
      $('#fileHomeWav').click()
    });
    $('#buttonJson3ok').on('click', (e: JQueryEventObject) => {
      this.closeModalJson3();
      this.generateHTML();
    });
    $('#buttonJson4ok').on('click', (e: JQueryEventObject) => {
      this.closeModalJson4();
      $("#filesWav").click()
    });
    $('#buttonInitialContinue').on('click', (e: JQueryEventObject) => {
      this.showModalJson1()
    });
  ...
  generateActionsAlbum() {
    ...
    $('.ui.segment.contentIconWav').on('mouseleave', (e: JQueryEventObject) => {
    ...
    $('.ui.segment.contentIconWav').on('mouseenter', (e: JQueryEventObject) => {
  ...
  generateSetingsOptionModal(): void {
    $('#buttonPlayAudioSemEfeito').on('mouseleave', (e: JQueryEventObject) => {
      this.sequenciador.stopOneSound()
    });

    $('#buttonPlayAudioSemEfeito').on('mouseenter', (e: JQueryEventObject) => {
      this.sequenciador.playOneSound(this.itemMixOption!.idBuffer, (function () { }))
    });
    $('#playAudioComOptions').on('mouseout', (e: JQueryEventObject) => {
      console.log("mouse out")
      this.sequenciador.stopOneSound()
    });

    $('#playAudioComOptions').on('mouseenter', (e: JQueryEventObject) => {
    ...
    $('#buttonVoiceComandModal').on('click', (e: JQueryEventObject) => {
      $('#buttonVoiceComandModal').toggleClass("active");
      this.voiceCommandModalOptions!.startRecognition(this.onEndRecordModalOptions)
    });
    $('#buttonCalcelModal').on('click', (e: JQueryEventObject) => {
    ...
    $('#buttonMinus').on('click', () => {
      if (this.itemMixOption!.getVolume() > 0) {
        $('#barVolume').progress('decrement', 10);
    ...
    $('#buttonPlus').on('click', () => {
      if (this.itemMixOption!.getVolume() < 200) {
        $('#barVolume').progress('increment', 10);
        this.itemMixOption!.setVolume(this.itemMixOption!.getVolume() + 10);
      }
    });
    ...
    let conteudoHTML = `
      ...
      <div class="ui form">
        <div class="field">
          ...
          <input id="nameFile" type="text" name="first-name" placeholder="First Name">
        </div>
      </div>
      </div>
      <div class="actions">
