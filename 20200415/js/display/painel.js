...
class Painel {
    ...
    actionMouseDown(event) {
        ...
        this.mouseDownX = (event.offsetX - 1);
        ...
        if (this.pageSoundSphereHome.panelReleased) {
            ...
            if ((this.mouseDownX != (evt.offsetX - 1)) || (this.mouseDownY != (evt.offsetY - 1))) {
                if (this.mouseDown) {
                    if (this.moved) {
                        let velocidade = 0.5;
                        const diferenceX = (evt.offsetX - 1) - this.firstPositionX;
                    ...
                    if (!this.moved) {
                        this.firstPositionX = (evt.offsetX - 1);
                    ...
                    if ((e.offsetX - 1) + this.displacingXAxis >= 0) {
                        var evt = e || event;
                        this.reMake();
                        this.ctxCanvas.fillText(this.sec2time(this.getSecondsByXPosition((e.offsetX - 1) + this.displacingXAxis)), (e.offsetX - 1) + this.displacingXAxis, (e.offsetY - 1) + this.displacingYAxis);
    ...
    getPositionX(event) {
        ...
        if ((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/Android/i)) || (navigator.userAgent.match(/iPod/i))) {
            ...
            x = ((event.offsetX - 1) !== undefined) ? (event.offsetX - 1) : (event.layerX - event.target.offsetLeft);
        }
        return x;
    }
    ...
    startLoopMarker(totalTime) {
        ...
        //Alterado para nao resetar o painel no eixo Y
        //this.resetTranslate();
        this.resetTranslateX();
