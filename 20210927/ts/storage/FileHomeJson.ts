class FileHomeJson extends FileJson {
    ...
    onReaderJson(evt: any): any {
        ...
        if (soundSphereDB.soundSphereInfo != undefined && soundSphereDB.soundSphereInfo.JSONFileStructureVersion == this.dao.soundSphereInfo.JSONFileStructureVersion) {
            ...
            this.simplePage.showErrorMessageJson1(`Aquivo incompatível carregue arquivos gerados pela versão SoundSphere - ${this.dao.soundSphereInfo.getFullName()}.`)
        }
    }


    handleFileSelectAudioToJSON(evt: any) {
    ...
    checkFileLoadedJSON(name: string) {
        // if (this.soundSphereBDbyJSONFile != undefined) {
        //     var bufferList = this.soundSphereBDbyJSONFile.bufferList;
        //     for (var i = 0; i < bufferList.length; i++) {
