function Sequenciador() {
    ...
    this.stopFlag = true;
    this.nameList = [];
    //contador do buffer list da funcao de loadBufferList
    this.contador = 0;
    this.index = 0;
...
Sequenciador.prototype.addBufferNull = function(name) {
    var itemBuffer = new ItemBuffer();
...
function continueDecode(bufferList, buffer, nameList) {
    if (buffer.numberOfChannels == 1 || buffer.numberOfChannels == 2 || buffer.numberOfChannels == 4 || buffer.numberOfChannels == 6) {
        sequenciador.bufferList[sequenciador.index] = sequenciador.addBuffer(buffer, sequenciador.nameList[sequenciador.contador]);
        console.log("Numer of chanels: " + buffer.numberOfChannels);
    } else {
        //melhorar na 1.5
        sequenciador.bufferList[sequenciador.index] = sequenciador.addBufferNull(sequenciador.nameList[sequenciador.contador]);
