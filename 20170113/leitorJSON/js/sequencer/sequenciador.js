...
//Funcao de play e pause dos audios do painel de opcoes
Sequenciador.prototype.stopOptionsPanel = function() {
    this.activeAudioOptionsPanel = false;
    this.audioOptionsPanel.stop(0);
};
Sequenciador.prototype.playOptionsPanel = function(id,volume, itemFilter, callBack) {
    console.log("Volume play options "+volume);
    if (!this.activeAudioOptionsPanel) {
        this.activeAudioOptionsPanel = true;
        var maxChannels = this.bufferList[id].numberOfChannels;
        var frameCount = this.audioCtx.sampleRate * this.bufferList[id].timeDuration;
        console.log("-- nunber of chanels: "+maxChannels);
        console.log("-- Time duration "+this.bufferList[id].timeDuration);
        console.log("-- frameCount "+frameCount );
        console.log("-- Volume "+frameCount );
        ...
        for (var channel = 0; channel < maxChannels; channel++) {
                ...
                var mix =this.bufferList[id].buffer.getChannelData(channel);
                for (var i = 0; i < mix.length; i++) {
                    //var r = Math.floor(i + (this.audioCtx.sampleRate * painel.listItemMixPanel[l][o].startTime));
                    //   v[r] += mix[i];
                    v[i] += (mix[i] * (volume / 100));
                }
        }
        this.audioOptionsPanel = this.audioCtx.createBufferSource();
        this.audioOptionsPanel.buffer = out;
        var filterLowPass = this.audioCtx.createBiquadFilter();
        filterLowPass.type = "lowpass";
        var filterHighPass = this.audioCtx.createBiquadFilter();
        filterHighPass.type = "highpass";
        // Get back to the frequency value between min and max.
        filterLowPass.frequency.value = itemFilter.lowpass.frequency;
        filterLowPass.Q.value = itemFilter.lowpass.Q;
        filterHighPass.frequency.value = itemFilter.highpass.frequency;
        filterHighPass.Q.value =itemFilter.highpass.Q;
        this.audioOptionsPanel.connect(filterLowPass);
        filterLowPass.connect(filterHighPass);
        filterHighPass.connect(this.audioCtx.destination);
        this.audioOptionsPanel.onended = function(e) {
            callBack();
            onEndSourceOptionsPanel();
        };
        this.audioOptionsPanel.start(0);
    }
};

///Funcao play e pause dos icones do menu princpal
Sequenciador.prototype.stopOneSound = function() {
    this.activeAudioIcon = false;
    this.audioIcon.stop(0);
};
Sequenciador.prototype.playOneSound = function(id, callBack, filters) {
...
//Funcoes que controlam quando e finalizado o audio ao terminar seu tempo de execucao
//para controlar se esta ativo ou nao
function onEndSource() {
    sequenciador.activeAudioIcon = false;
}
function onEndSourceOptionsPanel() {
    sequenciador.activeAudioOptionsPanel = false;
}
...
Sequenciador.prototype.play = function(bufferAudio) {
        save();
        controlPainel.disableOptionsItens();
...
function onEndPlayList() {
    if (sequenciador.activeLoop && (!sequenciador.stopFlag) && (!sequenciador.activePause)) {
        ...
        if (sequenciador.activePause) {
            ...
            controlPainel.enableOptionsItens();
        }
    }
}
