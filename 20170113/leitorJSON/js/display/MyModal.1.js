...
MyModal.prototype.setAttributesFilter = function () {
  console.log('setAttributesFilter teste');
...
MyModal.prototype.generateActionsFilter = function () {
  return function () {
    ...
    ${sequenciador.bufferList.length == 0 ?
        ...
        <button onclick="myModal.nextItemBuffer(); myModal.render()" class="ui yellow right labeled icon button">
          <i class="chevron right icon"></i>
          Mudar Amostra
        </button>`}
      <button onclick="setShowErorMessageToast();  files.click();" class="ui blue right labeled icon button">
...
MyModal.prototype.generateAttributesFilter = function () {
  return function () {
    ...
    filter.forEach(element => {
      ...
      conteudo += `
      <div class="contentFilter" id="container${element.type}" data-id="container${element.type}">
