...
function togle(id) {
  console.log("Selecionou id: " + id);
  console.log("---- id: " + id);
  var sgv = document.getElementById(id);
  var controlSoundIcon = new ControlSoundIcon();
  controlPainel.idSoundIconSelect = id;
  //Se for verdadeiro ele nao esta selecionado entao precisa remover de outro
  //icone se hovuer algum circulado, e depois inserir o cirulo de selecao
  //se for false ele esta selecionado entao deve-se remover o circulo
  if (sgv.childNodes.length == 1) {
    ...
    console.log("--- Remove o circle no: " + id);
    sgv.removeChild(circle);
    controlPainel.idSoundIconSelect = "";
    controlPainel.releaseOptions();
  }
};
//Funcao para verificar se tem algum Item do painel da biblioteca selecionado
//caso ele encontre ele retorna True caso nao encontre false
ControlSoundIcon.prototype.checkIconSoundSelected = function() {
  var container = document.getElementsByTagName('svg');
  var checkSoundSelect = false;
  for (var i = 0; i < container.length; i++) {
    if (container[i].childNodes.length > 1) {

      checkSoundSelect =  true;
      break;
    }
  }
  return checkSoundSelect;
}
