...
class PageAdminDescriptors extends SimplePage {
    ...
    generateAttributes() {
      ...
      <div style="margin-bottom:1px" class="ui  header">
        ...
        <span id="span1${element.type}${index}" ${element.status != true ? `class="span1Line" ` : ""}>
          ...
          <span ${element.status != true ? `class="span2Line" ` : ""} id="span2${element.type}${index}">${element.name}</span>
        </span>
    ...
    setSettingsActions() {
        ...
        $('#buttonPlayFilter').on('mouseout', (e) => {
            this.sequenciador.stopOneSound();
        });
        $('#buttonPlayFilter').on('mouseover', (e) => {
            this.setAttributesFilter();
        ...
        $('#buttonPlay').on('mouseout', (e) => {
            this.sequenciador.stopOneSound();
        });
        $('#buttonPlay').on('mouseover', (e) => {
            this.setAttributesFilter();
            this.sequenciador.playOneSound(this.controlSelectedItemBuffer, (function () { }));
        });
        $('#buttonChangeWav').on('click', (e) => {
            this.setAttributesFilter();
            this.nextItemBuffer();
            this.render();
        });
...
<div class="actions">
  ...
  `;
    ...
    generateSetingsModalNewFilter() {
        ...
        $("#select-AddFilter").on('change', (e) => {
            this.idFilterSelected = $(e.target).val();
            this.renderAddFilter();
        });
        $('#buttonCancelNewFilter').on('click', (e) => {
            this.closeModalNewFilter();
        });
        $('#buttonSaveNewFilter').on('click', (e) => {
            this.addFilter();
            this.closeModalNewFilter();
        });
    }
    addFilter() {
        if (this.newFilterModal.frequency !== undefined) {
            ...
            console.log($(`#addFrequency${this.newFilterModal.type}`).val());
