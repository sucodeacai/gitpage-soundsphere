...
class Painel {
    ...
    actionMouseMove(e) {
        if (this.pageSoundSphereHome.panelReleased) {
            ...
            if ((this.mouseDownX != evt.offsetX) || (this.mouseDownY != evt.offsetY)) {
                if (this.mouseDown) {
                    const sizeMoviment = 10;
                    this.deltaX = evt.offsetX - this.lastX;
                    //normalizaDelta para não ficar se movimentao mto
                    if (this.deltaX >= 1) {
                        this.deltaX = sizeMoviment;
                    }
                    if (this.deltaX <= -1) {
                        this.deltaX = -sizeMoviment;
                    }
                    this.deltaY = evt.offsetY - this.lastY;
                    //normalizaDelta para não ficar se movimentao mto
                    if (this.deltaY >= 1) {
                        this.deltaY = sizeMoviment;
                    }
                    if (this.deltaY <= -1) {
                        this.deltaY = -sizeMoviment;
                    }
    ...
    moveAction() {
        if (this.displacingXAxis >= 0 && this.displacingXAxis <= this.maxDisplacingXAxis) {
            if (this.displacingXAxis + (this.deltaX * -1) > this.maxDisplacingXAxis) {
                this.displacingXAxis = this.maxDisplacingXAxis;
            }
            else if (this.displacingXAxis + (this.deltaX * -1) < 0) {
                this.displacingXAxis = 0;
            }
            else {
        ...
        if (this.displacingYAxis >= 0 && this.displacingYAxis <= this.maxDisplacingYAxis) {
            if (this.displacingYAxis + (this.deltaY * -1) > this.maxDisplacingYAxis) {
                this.displacingYAxis = this.maxDisplacingYAxis;
            }
            else if (this.displacingYAxis + (this.deltaY * -1) < 0) {
                this.displacingYAxis = 0;
            }
            else {
