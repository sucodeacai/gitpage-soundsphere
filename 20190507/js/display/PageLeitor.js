      ...
      <td>${element.amount}</td>
        ...
        this.dao.sessionControl.listEventSession.forEach(element => {
            if (element.datDateSave) {
                ...
                const stringDataStart = `${dataStart.getDate()}/${(dataStart.getMonth() + 1)}/${dataStart.getFullYear()} - ${dataStart.getHours()}:${dataStart.getMinutes()}:${dataStart.getSeconds()} `;
                const stringDataSave = `${dataSave.getDate()}/${(dataSave.getMonth() + 1)}/${dataSave.getFullYear()} - ${dataSave.getHours()}:${dataSave.getMinutes()}:${dataSave.getSeconds()} `;
                dadosSession += `
        <tr>
        <td>${element.name}</td>
